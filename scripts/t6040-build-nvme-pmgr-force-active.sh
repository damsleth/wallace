#!/usr/bin/env bash
# Build, but do not boot, the T6040 PMGR force-active diagnostic.
set -euo pipefail

ROOT=$(cd "$(dirname "$0")/.." && pwd)
OUT=${OUT:-/Users/damsleth/Code/linux-build-out}
BUILD_DIR=${BUILD_DIR:-/build/linux-keyboard}
DTB=t6040-j614s-dcuart-nvme-pmgr-force-active.dtb
INITRAMFS=initramfs-dcuart-nvme-pmgr-force-active.cpio.gz
...
for src in \
    ...
    t6040-j614s-dcuart-nvme-pmgr-force-active.dts; do
...
EXTRA_FILES="$OUT/nvme-core-pmgr-force-active.ko:lib/modules/nvme-core.ko $OUT/nvme-apple-pmgr-force-active.ko:lib/modules/nvme-apple.ko" \
DEST="$OUT/$INITRAMFS" \
    "$ROOT/scripts/t6040-make-initramfs.sh"

shasum -a 256 \
    "$OUT/Image-nvme-pmgr-force-active" \
    "$OUT/$DTB" \
    "$OUT/$INITRAMFS" \
