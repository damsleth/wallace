#!/usr/bin/env bash
# Build, but do not boot, the T6040 force-active NVMe continuation diagnostic.
set -euo pipefail
...
INITRAMFS=initramfs-dcuart-nvme-force-continue.cpio.gz
...
EXTRA_FILES="$OUT/nvme-core-force-continue.ko:lib/modules/nvme-core.ko $OUT/nvme-apple-force-continue.ko:lib/modules/nvme-apple.ko" \
DEST="$OUT/$INITRAMFS" \
    "$ROOT/scripts/t6040-make-initramfs.sh"

shasum -a 256 \
    "$OUT/Image-nvme-force-continue" \
    "$OUT/$DTB" \
    "$OUT/$INITRAMFS" \
