...
|---|---|---|
| 004 trackpad motion | **Superseded by 125/126 (2026-07-24 correction).** The first exact candidate was independently rejected before boot because it lacked the HID type fix and left multitouch modular without modules. Offline 125 later byte-reproduced and passed review. See `evidence/2026-07-24-t6040-trackpad-motion-revised-preflight.md` | Live 126 needs fresh approval, volatile-runtime-HIDF exception, and attended motion |
...
Post-review correction: the 11–21 July `#asahi-dev` audit established that the
DockChannel UART is on AIC input 816, not ADT input 360. Rebuild the one-port
poll-mode USB DTBs with the corrected interrupt before the smoke test. Poll mode
does not request the line, so this is a correctness/provenance rebuild rather
than a new runtime variable. Full record:
`evidence/2026-07-21-asahi-dev-log-review.md`.
